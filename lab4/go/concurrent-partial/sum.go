...
func compareFiles(file1, file2 string, fingerprint1, fingerprint2 []int, results chan<- string) {
	similarityScore := similarity(fingerprint1, fingerprint2)
	result := fmt.Sprintf("Similarity between %s and %s: %.2f%%\n", file1, file2, similarityScore*100)
	results <- result
}
...
func main() {
	...
	for i := 0; i < len(os.Args)-1; i++ {
		for j := i + 1; j < len(os.Args)-1; j++ {
			...
			totalComparisons++
			go compareFiles(file1, file2, fingerprint1, fingerprint2, results)
		}
	}
